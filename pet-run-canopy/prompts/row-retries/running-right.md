Create Codex pet row `running-right` for `canopy`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Compact friendly woodland data-sprite for a coding-and-forest-plotting workspace: moss-green leaf hood, warm bark-brown body, tiny chart-leaf satchel, bright curious eyes, calm focused helper energy, no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
